defmodule GamificationWeb.AchievementLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      >
        ...
        <.input field={@form[:points_awarded]} type="number" label="Points awarded" />
        <:actions>
  ...
  def update(%{achievement: achievement} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Achievements.change_achievement(achievement))
     end)}
  end

  @impl true
  def handle_event("validate", %{"achievement" => achievement_params}, socket) do
    changeset = Achievements.change_achievement(socket.assigns.achievement, achievement_params)
  ...
  defp save_achievement(socket, :edit, achievement_params) do
    case Achievements.update_achievement(socket.assigns.achievement, achievement_params) do
      {:ok, achievement} ->
