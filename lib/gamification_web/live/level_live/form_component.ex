defmodule GamificationWeb.LevelLive.FormComponent do
  ...
  def update(%{level: level} = assigns, socket) do
    {:ok,
     ...
     |> assign(assigns)
     |> assign_new(:form, fn ->
       to_form(Levels.change_level(level))
     end)}
  end

  @impl true
  def handle_event("validate", %{"level" => level_params}, socket) do
    changeset = Levels.change_level(socket.assigns.level, level_params)
  ...
  defp save_level(socket, :edit, level_params) do
    case Levels.update_level(socket.assigns.level, level_params) do
      {:ok, level} ->
  ...
  defp save_level(socket, :new, level_params) do
    case Levels.create_level(level_params) do
      {:ok, level} ->
        notify_parent({:saved, level})
