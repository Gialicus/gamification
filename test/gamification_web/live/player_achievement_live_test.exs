defmodule GamificationWeb.PlayerAchievementLiveTest do
  ...
  @create_attrs %{achieved_at: "2025-02-22T22:04:00"}
  @update_attrs %{achieved_at: "2025-02-23T22:04:00"}
  ...
  describe "Index" do
    ...
    test "lists all player_achievements", %{conn: conn} do
      {:ok, _index_live, html} = live(conn, ~p"/player_achievements")

      assert html =~ "Listing Player achievements"
    end

    test "saves new player_achievement", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, ~p"/player_achievements")

      assert index_live |> element("a", "New Player achievement") |> render_click() =~
               "New Player achievement"

      assert_patch(index_live, ~p"/player_achievements/new")
      ...
      assert_patch(index_live, ~p"/player_achievements")
    ...
    test "updates player_achievement in listing", %{conn: conn, player_achievement: player_achievement} do
      ...
      assert index_live |> element("#player_achievements-#{player_achievement.id} a", "Edit") |> render_click() =~
               "Edit Player achievement"

      assert_patch(index_live, ~p"/player_achievements/#{player_achievement}/edit")
    ...
    test "deletes player_achievement in listing", %{conn: conn, player_achievement: player_achievement} do
      ...
      assert index_live |> element("#player_achievements-#{player_achievement.id} a", "Delete") |> render_click()
      refute has_element?(index_live, "#player_achievements-#{player_achievement.id}")
    end
  end
  ...
  describe "Show" do
    ...
    test "displays player_achievement", %{conn: conn, player_achievement: player_achievement} do
      {:ok, _show_live, html} = live(conn, ~p"/player_achievements/#{player_achievement}")
    ...
    test "updates player_achievement within modal", %{conn: conn, player_achievement: player_achievement} do
      {:ok, show_live, _html} = live(conn, ~p"/player_achievements/#{player_achievement}")

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit Player achievement"

      assert_patch(show_live, ~p"/player_achievements/#{player_achievement}/show/edit")
      ...
      assert_patch(show_live, ~p"/player_achievements/#{player_achievement}")
